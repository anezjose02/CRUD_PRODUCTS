@php
    ...
    $facturas = Facturas::select('id')->where('num_fac', $id)->first();
    //dd($facturas);
    $compras = Compras::select('id_user')->where('id', $facturas->id)->first();
    $user = DB::table('users')->where('id', $compras->id_user)->first();
    //dd($user);
    $subTotal = 0;
@endphp
<x-app-layout>
                                            ...
                                            <td class="info_cliente">
                                                <div class="round">
                                                    ...
                                                    <table class="datos_cliente">
                                                        ...
                                                        @php
                                                                $compras_realizadas = Facturas::select('*')->where('num_fac', $id)->get();
                                                        @endphp
                                                        <tr>
                                                            
                                                            <td><label>Nº Compras:</label>{{count($compras_realizadas)}} </td>
                                                            <td><label>Nº Factura:</label>{{$id}} </td>
                                    ...
                                    <table id="factura_detalle">
                                        <span class="h3">Detalle de la Factura</span>
                                            ...
                                            <tbody id="detalle_productos">
                                                @foreach ($compras_realizadas as $item)
                                                @php
                                                    $cant_articulos = Compras::select('*')->where('id', $item->id)->get();
                                                @endphp
                                                    @foreach ($cant_articulos as $value)
                                                        @php
